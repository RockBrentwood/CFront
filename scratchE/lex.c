...
lex.c:
	...
	error[T][L]() is used to report errors
	{} and () must match
...
#define retn(a,b)	{ yylval.p = (NodeP)b; return lastseen=a; }
...
void lxenter(register char *s, register short m) {
   register int c;
                  ...
                  case '"':
                     ...
                     /*      maintain stack of file names */
                        ...
                        /* new file name: push */
                         ...
                         push:
                           Nfile++;
                           p = _new((txtfree - txtstart + 1)*sizeof *p);

                           if (MAXFILE <= ++f) errorT('i', "fileN stack overflow"); //(#) Clipped at "fileN ".
...
/*
					int f = curloc.file;
					if (1<f) errorT('i',"fileN buffer (%d)",f);
					if (f) _delete file_name[f];
*/
                        curloc.file = 0;
                     }
                     del_txt();
