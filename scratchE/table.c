...
/*
	the struct Table functions assume that new initializes store to 0
*/
...
/*
	...
	both the vector of struct Id pointers and the hash table
...
TableP MakeTable(short sz, TableP nx, IdP n) {
   TableP this = (TableP)_new(sizeof *this);
   this->base = TABLE;
   this->t_name = n;
   ...
   this->next = nx;
   this->free_slot = 1;
/* fprintf(stderr,"MakeTable %d %s %d\n", this, (n)?n->string:"?", this->size); fflush(stderr);*///(#) Clipped at "fflush(s".
   return this;
}
...
*/
IdP look(TableP this, char *s, Token k) {
   TableP t;
   register char *p;
   register char *q;
   register int i;
   ...
   if (s == 0) errorT('i', "%d->look(0)", this);
   if (this == 0) errorT('i', "0->look(%s)", s);
   if (this->base != TABLE) errorT('i', "(%d,%d)->look(%s)", this, this->base, s);
...
/* use simple hashing with linear search for overflow */
   ...
   /* in this and all enclosing scopes look for name "s" */
    ...
    found:
      for (; n; n = n->n_tbl_list) { /* for  all name "s"s look for a key match *///(#) Clipped at "for a ke".
...
IdP insert(TableP this, IdP nx, Token k) {
   ...
   if (s == 0) errorT('i', "%d->insert(0,%d)", this, k);
   nx->n_key = k;
...
/* fprintf(stderr,"tbl.grow %d %s %d->%d\n", this, (this->t_name)?this->t_name->string:"?", this->size, g+1); fflush(stderr); *///(#) Clipped at ", this->size".
   this->size = mx = g + 1;
...
*/
IdP lookc(TableP this, char *s, Token tk) {
   TableP t;
   register char *p;
   register char *q;
   register int i;
   ...
   if (s == 0) errorT('i', "%d->lookc(0)", this);
   if (this == 0) errorT('i', "0->lookc(%s)", s);
   if (this->base != TABLE) errorT('i', "(%d,%d)->lookc(%s)", this, this->base, s);
   ...
   /* in this and all enclosing scopes look for name "s" */
    ...
    not_found:
      if (tname) {
         ...
         if (cl && cl->clbase && cl->pubbase == 0) Ebase = (ClassP) cl->clbase->tp; //(#) Clipped at "(ClassP)cl-".
      }
   }
...
*/
void new_key(char *s, Token toknum, Token yyclass) {
   IdP n = MakeId(s);
