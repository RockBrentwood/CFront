...
IdP dcl_local(TableP scope, IdP an, IdP fn) {
   if (scope == 0) {
      errorT('s', "cannot expand inlineF needing temporary variable in nonF context"); //(#) Clipped at "in nonF".
      return an;
   }
...
StP expandSt(StP this) {
   if (this == 0) errorT('i', "0->expandSt() for%n", expand_fn);
/*errorT('d',"stmt %d:%k s=%d e=%d l=%d",this,this->base,this->s,this->e,this->s_list);*/
...
/*fprintf(stderr,"%s(): expr %d: b=%d e1=%d e2=%d\n",expand_fn->string,this,this->base,this->e1,this->e2); fflush(stderr);*///(#) Clipped at "base,e1,".
   switch (this->base) {
      case NAME:
      ...
      case ICALL:
         if (expand_tbl && this->e1 == 0) {
            ...
            if (f->returns == (TypeP)void_type && fn->n_oper != CTOR)
               errorT('s', "non-value-returning inline%n called in value-returning inline%n", fn, expand_fn); //(#) Clipped at "called in v".
            else
   ...
   switch (this->base) {
      ...
      case ANAME:
         if (curr_icall) {
            ...
            goto bok;
          aok:
            return (il->local[argno]) ? 0 : not_simple(il->arg[argno]); //(#) Clipped at "not_simple(".
         }
...
/*errorT('d',"expand%n inline=%d last_exp=%d curr_expr=%d",fn,this->f_inline,last_expanded,curr_expr);*///(#) Clipped at "last_expanded,".
   if ((this->body == 0 && this->f_expr == 0) /* called before defined */
      ||(((FunP) fn->tp)->body->memtbl == scope) /* called while defining */
      ||(this->f_inline == 2) /* recursive call */
      ||(this->last_expanded && this->last_expanded == curr_expr)
   /* twice in an expression *///(#) Clipped at "expression *".
      ) {
    ...
    */
      if (ll == 0) errorT('i', "expandFun:AX for %n", fn);
      ExP ee;
   ...
   /*errorT('d',"n=%n addr %d ass %d used %d s %d",n,n->n_addr_taken,n->n_assigned_to,n->n_used,s);*///(#) Clipped at "n->n_assigned_to,n-".
      if (n->n_addr_taken || n->n_assigned_to) {
         IdP nn = dcl_local(scope, n, fn);
         nn->base = NAME;
         il->local[i] = nn;
         ++NotSimple;
      } else if (n->n_used && (s = not_simple(ee)) && (1 < s || 1 < n->n_used)) { /* not safe */
         IdP nn = dcl_local(scope, n, fn);
         nn->base = NAME;
         il->local[i] = nn;
         ++NotSimple;
   ...
   /*errorT('d',"NotSimple=%d loc_var=%d last_expanded=%d curr_expr=%d",NotSimple,loc_var,this->last_expanded,curr_expr);*///(#) Clipped at "loc_".
...
/*errorT('d',"%n = %k",n,e->base);*/
      ...
      if (NotSimple) {
         ...
         for (i = 0; i < MIA; i++) {
            ...
            if (--NotSimple)
               stail = stail->s_list = (StP)MakeESt(SM, curloc, 0, 0); //(#) Clipped at "SM,curloc".
            else
               break;
         }
