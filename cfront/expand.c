...
{
	if (scope == 0) {
		error('s',"cannot expand inlineF needing temporary variable in nonF context"); //(#) Clipped at "in nonF".
		return an;
	}
...
/*fprintf(stderr,"%s(): expr %d: b=%d e1=%d e2=%d\n",expand_fn->string,this,base,e1,e2); fflush(stderr);*/ //(#) Clipped at "base,e1,".
	switch (base) {
	case NAME:
	...
	case ICALL:
		if (expand_tbl && e1==0) {
			...
			if (f->returns==void_type && fn->n_oper!=CTOR)
				error('s',"non-value-returning inline%n called in value-returning inline%n",fn,expand_fn); //(#) Clipped at "called in v".
			else
...
/*error('d',"not_simple%k",base);*/
	...
	case ANAME:
		if (curr_icall) {
			...
			goto bok;
		aok:
			return (il->local[argno]) ? 0 : il->arg[argno]->not_simple(); //(#) Clipped at "not_simple(".
		}
...
*/
{
/*error('d',"expand%n inline=%d last_exp=%d curr_expr=%d",fn,f_inline,last_expanded,curr_expr);*/ //(#) Clipped at "last_expanded,".
...
/*error('d',"n=%n addr %d ass %d used %d s %d",n,n->n_addr_taken,n->n_assigned_to,n->n_used,s);*/ //(#) Clipped at "n->n_assigned_to,n-".
		if (n->n_addr_taken
		|| n->n_assigned_to) {
...
/*error('d',"not_simple=%d loc_var=%d last_expanded=%d curr_expr=%d",not_simple,loc_var,last_expanded,curr_expr);*/ //(#) Clipped at "loc_".
...
/*error('d',"%n = %k",n,e->base);*/
		...
		if (not_simple) {
			...
			for (i=0; i<MIA; i++) {
				...
				if (--not_simple)
					stail = stail->s_list = new estmt(SM,curloc,0,0); //(#) Clipped at "SM,curloc".
				else
					break;
			}
			stail->s_list = body;
