...
/*
	...
	Avoid (illegal) repetition of basetypes which are class or enum declarations //(#) Clipped at "declaration".
...
*/
{
	...
	for (n=this; n; n=n->n_list) {
		...
		case ASM:
			fprintf(out_file,"asm(\"%s\")\n",(char*)((Pbase)t)->b_name); //(#) Clipped at "b_name)".
			break;
				...
				if (i) {
					...
					cmp:
						...
						default:
							...
							break;
						case TYPE:	
							t1 = ((Pbase)t1)->b_name->tp; //(#) Clipped at "b_name->t".
							goto cmp;
						case VEC:
							if (((Pvec)t1)->typ->base==CHAR) { //(#) Clipped at "typ_base==".
								i->print();
								break;
							}
						case PTR:
	...
	/*
		...
		if (tbl)
			for (nn=tbl->get_mem(i=1); nn; nn=tbl->get_mem(++i) )
				if (nn->n_scope==ARGT && nn->n_union==0) nn->dcl_print(SM); //(#) Clipped at "nn->dcl_pr". Not verified.
	*/
...
/*fprintf(stderr, "mem %d %s %d union %d tp %d %d\n", nn, nn->string, nn->base, nn->n_union, nn->tp, nn->tp->base);*/ //(#) Clipped at "nn->base, nn->". Not verified.
			if (nn->base==NAME && nn->n_union==0) {
				if (nn->tp->base == CLASS) ((Pclass)nn->tp)->dcl_print(nn); //(#) Clipped at "dcl_pr". Not verified.
			}
		}
	}
	...
	if (print_mode == SIMPL) {
		...
		if (virt_count) {	/* print initialized jump-table */
			...
			for (nn=memtbl->get_mem(i=1); nn; nn=memtbl->get_mem(++i) ) { //(#) Clipped at "gem_mem(++i) )".
...
void expr.print()
{
	...
	if (this==e1 || this==e2) error('i',"(%d%k)->expr.print(%d %d)",this,base,e1,e2); //(#) Clipped at "this,base,e".
	...
	case ANAME:
		...
		aok:
			if (n = il->local[argno])
				...
				Ptype t = il->tp[argno];
				if (ee==0 || ee==this) error('i',"%d->expr.print(A %d)",this,ee); //(#) Clipped at "expr.print(A ".
...
/*fprintf(stderr,"at %s tp (%d %d)\n", at->string?at->string:"?", t, t?t->base:0);*/
					if (e == 0) error('i',"A missing for %s()",(fn)?fn->string:"??"); //(#) Clipped at 'for %s()",'.
					if (e->base == ELIST) {
						ex = e->e1;
						e =  e->e2;
					}
					...
					if (t!=ex->tp && t->is_cl_obj()==0 && eobj==0) { //(#) Clipped at "eobj=".
						puttok(LP);
						{	bit oc = Cast;
							Cast = 1;
							t->print();
	...
	bkk:
		eprint(e1);
		puttok(base);
		if (e1->tp!=e2->tp && e2->base!=ZERO) {	/* cast, but beware of int!=long etc */ //(#) Clipped at "of int!".
			Ptype t1 = e1->tp;
		cmp:
			switch (t1->base) {
			default:	break;
...
/*error('d',&where,"stmt.print %d:%k s %d s_list %d",this,base,s,s_list);*/
	...
	if (memtbl && base!=BLOCK) { /* also print declarations of temporaries */
		...
		for (n=tbl->get_mem(i=1); n; n=tbl->get_mem(++i)){
			...
			Pname cn;
			if (bl && (cn=n->tp->is_cl_obj()) && Pclass(cn->tp)->has_dtor()) bl = 0; //(#) Clipped at "has_dt".
		}
...
/*if (e->base==CALL || e->base==G_CALL) error('d',"%n",(Pname)e->e1);*/
		if (e) {
			e->print();
			if (e->base==ICALL && e->e2) break;	/* a block: no SM */ //(#) Clipped at "no SM *".
		}
		puttok(SM);
		break;
	case WHILE:
...
//error('d',"arg %d%k",arg,arg->base);
		...
		&& e->e2->base==ANAME) {
			...
			Pname a1 = (Pname)e->e1;
			Pname a2 = (Pname)e->e2;
/*error('d',"aname%n %d %d%n %d %d",a1,a1->n_val,a1->n_table,a2,a2->n_val,a2->n_table);*/ //(#) Clipped at "a2->n_tabl". Not verified.
			Pexpr arg2 = aval(a2);
