...
*/
{
	...
	next = nx;
	free_slot = 1;
/* fprintf(stderr,"table.table %d %s %d\n", this, (n)?n->string:"?", size); fflush(stderr);*/ //(#) Clipped at "fflush(s".
} 
...
void table.grow(int g)
{
	...
	if (g <= size) return;
/* fprintf(stderr,"tbl.grow %d %s %d->%d\n", this, (t_name)?t_name->string:"?", size, g+1); fflush(stderr); */ //(#) Clipped at ", size".
	size = mx = g+1;
...
*/
{
	...
	not_found:
		if (tname) {
			...
			if (cl && cl->clbase && cl->pubbase==0) Ebase = (Pclass)cl->clbase->tp; //(#) Clipped at "(Pclass)cl-".
		}
	}
