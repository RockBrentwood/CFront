...
#define CHECKTRAP()	if (t_trap != *(t_basep-t_stacksize+1)) task_error(E_STACK,0) //(#) Clipped at "STACK,".
...
/* xxx changed from ifdef vax */
	...
	case SHARED:
		...
		if (mode==0 && stacksize==0)
			t_stacksize = thistask->t_stacksize - (thistask->t_basep - t_basep); //(#) Clipped at "->t_basep - ".
		thistask = this;
		return;
	default:
...
*/
{
	...
	if (n&STACK) {
		...
		if (s_state == TERMINATED) {
			...
			int b = (int) t_basep;
			int x = ((this==thistask) || t_mode==DEDICATED) ? b-(int)t_framep : t_size; //(#) Clipped at "b-(int)t_".
			printf("max=%d current=%d",t_stacksize,x);
			if (_hwm) printf(" hwm=%d",curr_hwm());
			printf(" t_base=%d, t_frame=%d, t_size=%d\n",b,t_framep,t_size); //(#) Clipped at "t_framep,t_s".
		}
	}
