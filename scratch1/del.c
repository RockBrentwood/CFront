...
void delId(IdP this) {
/*fprintf(stderr,"%d->Id::del: %s %d\n",this,(this->string)?this->string:"?",this->base);fflush(stderr);*/
...
//fprintf(stderr,"DEL(Type, type=%d %d)\n",this,this->base);
   this->permanent = 3; /* do not delete twice */
   switch (this->base) {
      case TNAME:
      case NAME:
         errorT('i', "delType(%d):N %s %d", this, ((IdP) this)->string, this->base);
      case TYPE:
      {
         BaseP b = (BaseP) this;
         break;
...
void delTable(TableP this) {
   register int i;
/*fprintf(stderr,"Table::del %s %d size=%d used=%d)\n", (this->t_name)?this->t_name->string:"?", this, this->size, this->free_slot-1); fflush(stderr);*/
