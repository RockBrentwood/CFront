...
char *txtstart = 0;
char *txtfree = 0;
#define pch(c) ((txtmax<=txtfree)?errorT('i',"input buffer overflow"):(*txtfree++=c))
...
struct Loc curloc;
FILE *out_file/* = stdout*/; /* Moved to the start of main() */
FILE *in_file/* = stdin*/; /* Moved to the start of main() */
TableP ktbl;
...
extern int lxtitle(void);

// overload rt;
inline YYSTYPE rtS(char *x) {
   YYSTYPE y;
   y.s = x;
   return y;
}
inline YYSTYPE rtT(Token x) {
   YYSTYPE y;
   y.t = x;
   return y;
}
inline YYSTYPE rtL(struct Loc x) {
   YYSTYPE y;
   y.l = x;
   return y;
}
inline YYSTYPE rtN(void *x) {
   YYSTYPE y;
   y.pn = (IdP) x;
   return y;
}
...
#define reti(a,b)	{ addtok(a, rtT((Token)b));     return; }
#define retn(a,b)	{ addtok(a, rtN((NodeP)b));   return; }
#define rets(a,b)	{ addtok(a, rtS((char *)b));  return; }
#define retl(a)		{ addtok(a, rtL(curloc));     return; }
...
void putline(LocP this) {
   ...
   if (0 <= this->file && this->file < MAXFILE) {
      const char *f = file_name[this->file];
...
typedef struct LxDope *LxDopeP;
struct LxDope {
   short lxch; /* the character */
   short lxact; /* the action to be performed */
   Token lxtok; /* the token number to be returned */
} lxdope[] = {
...
void hex_to_oct(void) {
   ...
   if (0777 < i) errorT('s', "hexadecimal constant too large");
...
void tlex(void) {
   ...
   while (1) {
      ...
      switch ((p = lxcp[lxchar + 1])->lxact) {
         ...
         case A_LC:
            if (BLMAX <= bl_level++) {
               errorT('s', "blocks too deaply nested");
               ext(3);
            }
            retl(LC);
         ...
         case A_MUL:
            switch (get(lxchar)) {
               case '=':
                  reti(ASOP, ASMUL);
               case '/':
                  errorT('w', "*/ not as end of comment");
               default:
                  saved = lxchar;
                  reti(MUL, MUL);
            }
         ...
         default:
            errorT('i', "lex act==%d getc()->%d", p, lxchar);

      }

      errorT('i', "lex, main switch");
                  ...
                  case '"':
                     ...
                     if (*txtstart) { // stack file name
                        ...
                        // previous file: pop
                         ...
                         push:
                           if (MAXFILE * 4 < Nfile++) errorT('i', "fileN buffer overflow");
                           if (MAXFILE < curr_file++) errorT('i', "fileN stack overflow");
                           file_stack[curr_file] = Nfile;

                           char *p = _new((txtfree - txtstart + 1)*sizeof *p);
